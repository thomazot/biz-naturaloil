...
function calcBoxMenu($) {
    const headerWidth = $('.header').outerWidth()
    const logoWidth = $('.header .logo').outerWidth()
    const categories = $('.header .categories')

    categories.get(0).style.setProperty('--left', logoWidth + 'px')
    categories
        .get(0)
        .style.setProperty('--width', headerWidth - logoWidth + 'px')
}

function menuTitle($) {
    const items = $('.header .categories .li--0')

    items.each(function () {
        const title = $.trim($(this).find('.a--0').text())
        $(this).find('.ul--1').attr('data-title', title)
    })
}

function search($) {
    const searchEl = $('.header .search')
    const menuHeaderWidth = $('.header .categories').outerWidth()
    const searchWidth = searchEl.outerWidth()
